...
- **新規コード**（特に `src/domain/`・`src/services/`・`src/components/`）は、共有の DSL 型に **`src/domain/dsl-types.ts`** を正とする。
...
- **新規テスト**では `setup.js` の **グローバルフック（`Module.prototype.require` 等）への依存を増やさない**こと。方針の正本: `docs/test-setup-policy.md`。
