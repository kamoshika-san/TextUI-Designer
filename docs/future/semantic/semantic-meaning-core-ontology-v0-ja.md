...
> **compare-logic v2 境界注記（T-20260419-035）**:
> - **ラベル変更のみで 5 軸全同値** → compare-logic v2 の対象外（out-of-scope）。`component_label_changed` は発行しない。
> - **ラベル変更が 5 軸（action / availability / guard / state / transition）を非同値にする** → 既存 diff_event（`component_action_changed` / `component_availability_changed` / `component_guard_changed` / `entity_state_changed` / `transition_edge_changed`）で捕捉する。ラベル変化は根拠として `before_predicate` / `after_predicate` に記録できる。
> - `component_label_changed` は compare-logic v2 の closed vocabulary に含めない（設計G P2-3 / PM 決定）。
