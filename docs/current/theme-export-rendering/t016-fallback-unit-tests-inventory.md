...
| ファイル（変更後） | 分類 | 備考 |
|--------------------|------|------|
| `tests/unit/html-exporter-fallback-style-lane.test.js` | **必須** | Primary は React レンダリングのため、**互換用静的 CSS / セマンティッククラス**は `withExplicitFallbackHtmlExport` 経由でのみ実機検証できる。 |
...
| `tests/unit/html-exporter-lane-observability.test.js` | **必須** | **ログ／警告**の観測のみ。`html-exporter-fallback-style-lane`（マークアップ／CSS）と重複しない。 |
