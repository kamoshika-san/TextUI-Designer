...
外部アーキ **Phase 5** の方針メモ。大規模な責務分割の **詳細は別チケット**。**T-20260422-015** で pass-through executor 層を撤去し、`ExportManager` → `export-pipeline` 純関数の2層に圧縮済み。
...
`T-20260422-015` で pass-through 層だった `OptimizingExportExecutor` を撤去し、`ExportManager` が `ExportPipelineDeps` を生成して `export-pipeline` の `runOptimizedExport` / `runExportWithDiffUpdate` を直接呼び出す形へ圧縮した。キャッシュ／diff／メトリクス記録の意味論は `export-pipeline`・[export-instrumentation.md](export-instrumentation.md) の既存契約を変えない。

## 現状の整理（コードベース）

`ExportManager`（`src/exporters/export-manager.ts`）は概ね次を **同一クラス内で合成**している。重い本流処理は `export-pipeline` の純関数へ委譲し、中間 executor class は置かない。
