...
| Explicit fallback entry | Helper stays in exporters; **CLI capture is Primary-default**（T-010） | `src/exporters/internal/fallback-lane-options.ts`（**内部互換 API**: [t017-html-export-lane-options-internal-api.md](./t017-html-export-lane-options-internal-api.md)） |
