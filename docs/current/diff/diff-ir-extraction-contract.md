...
The extraction layer outputs one comparison-ready IR payload whose vocabulary follows `docs/current/diff/diff-ir-vocabulary.md`.
...
- Confirm each IR unit defined in `docs/current/diff/diff-ir-vocabulary.md` can be reached from a DSL extraction path without introducing normalization logic.
