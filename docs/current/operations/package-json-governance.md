...
- `extract-zip` is a direct `file:vendor/extract-zip` pin (local 2.0.3) plus `"overrides": { "extract-zip": "$extract-zip" }` because npm has no patched release for CVE-2026-56876 / GHSA-jmr9-qjv8-65gv, and `puppeteer-core` 24 must stay on CommonJS / Node 18–20. The `$extract-zip` form is required so the `file:` spec resolves from the repo root rather than from `@puppeteer/browsers`. Include `vendor/extract-zip/**/*` in `files` so the `file:` dependency packs. The vendor copy resolves symlink targets against `fs.realpath` of the destination parent and refuses regular-file writes through existing symlinks.
