...
- `extract-zip` is a direct `file:vendor/extract-zip` pin (local 2.0.4) plus `"overrides": { "extract-zip": "$extract-zip" }` because npm has no patched release for CVE-2026-56876 / GHSA-jmr9-qjv8-65gv, and `puppeteer-core` 24 must stay on CommonJS / Node 18–20. The `$extract-zip` form is required so the `file:` spec resolves from the repo root rather than from `@puppeteer/browsers`. Include `vendor/extract-zip/**/*` in `files` so the `file:` dependency packs. The vendor copy walks symlink targets through existing intermediate links (not lexical `path.resolve`), re-checks with `fs.realpath.native` after create, and creates directories only after the existing prefix stays inside the extract dir.
- `yauzl` is a direct `3.2.1` pin plus `"overrides": { "yauzl": "$yauzl" }` so the file-linked `extract-zip` cannot pull `yauzl@2.10.0` and diverge from `npm ci`.
