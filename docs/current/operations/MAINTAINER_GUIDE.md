...
- 棚卸し・現況の正本: `docs/current/dsl-ssot-types/dsl-types-renderer-types-inventory.md`（`npm run check:dsl-types-ssot` のスナップショット節と **同一**に保つ。更新手順は同文書「棚卸し文書の更新手順」）
- 日次運用手順の正本: `docs/current/workflow-onboarding/adding-built-in-component.md`
- DSL 型追加の最短導線: `docs/current/dsl-ssot-types/ssot-dsl-type-addition-rules.md`
...
- sprint 途中の候補調査メモや PoC メモは backlog の正本ではない。現在の判断面は `docs/current/dsl-ssot-types/ssot-renderer-facade-sprint3-decision.md` と `docs/current/dsl-ssot-types/ssot-renderer-components-batching-memo.md` を優先する。
...
| [documentation-doc-debt-runbook.md](documentation-doc-debt-runbook.md) | docs の stale / duplicate / broken link / outdated guidance を月次で点検する運用手順 |
...
- renderer SSoT まわりの docs 更新では、`docs/current/dsl-ssot-types/dsl-types-renderer-types-inventory.md`、`docs/current/testing-ci/ssot-import-guard-matrix.md`、`docs/current/dsl-ssot-types/ssot-renderer-facade-sprint3-decision.md` の current snapshot 記述を相互にずらさない。
...
| **HtmlExporter（HTML 出力の primary-only 契約）** | **Primary（既定）**: `useReactRender !== false` → `react-static-export`。`useReactRender === false` は `[HtmlExporter:FALLBACK_REMOVED]` で拒否。詳細は [exporter-boundary-guide.md](exporter-boundary-guide.md)「HtmlExporter」・[export-contract.md](../runtime-boundaries/export-contract.md) | `npm run compile` + 既存 exporter 系ユニット |
| **テーマ token → CSS（プレビューとエクスポートの対応）** | **正本（kebab）**: `src/components/definitions/exporter-renderer-definitions.ts` の `tokenStyleProperty` → 集約マップ `src/components/definitions/token-style-property-map.ts`。**エクスポート**: `src/exporters/legacy/base-component-renderer.ts`（`getTokenStylePropertyKebab`）。**WebView プレビュー**: `src/renderer/token-inline-style-from-definition.ts` の `tokenToPreviewInlineStyle`（各 `src/renderer/components/*.tsx`）。`Spacer` は寸法フォールバックとして token を別用途で使うためマップと完全一致ではない。 | `npm run compile` + `npx mocha ... tests/unit/token-style-property-map.test.js` |
...
| サービス初期化順・cleanup（宣言フェーズ） | `src/services/service-runtime-phases.ts` + `src/services/service-initializer.ts`（意図の短い説明: [service-design-service-initializer.md](service-design-service-initializer.md)） | `npm run compile` + `npx mocha ... tests/unit/service-initializer.test.js` + `extensibility-service-factories-contract.test.js`（詳細は `docs/current/services-webview/service-registration.md`） |
...
| 拡張 activate/deactivate の順序 | `src/services/extension-lifecycle-phases.ts` + `src/services/extension-lifecycle-manager.ts` | `npm run compile` + `npx mocha ... tests/unit/extension-lifecycle-phases.test.js`（全体は `docs/current/services-webview/service-registration.md`） |
...
- **CLI / MCP / services から `src/exporters/internal/**`（fallback 互換 API）を直接 import しない**（T-010 以降、capture も Primary 既定。T-020）。正本: [t017-html-export-lane-options-internal-api.md](../theme-export-rendering/t017-html-export-lane-options-internal-api.md)。
