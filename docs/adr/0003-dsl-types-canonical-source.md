...
- `src/domain/dsl-types/（公開エントリ: index.ts）` と `src/renderer/types.ts` が **同一の DSL 概念**（例: `TextUIDSL`, `ComponentDef`）について **並行した型定義**を持っている。
...
1. **共有 DSL 型の正本は `src/domain/dsl-types/（公開エントリ: index.ts）`** とする（`TextUIDSL` / `ComponentDef` およびそれに紐づく構造）。
...
- 共有 DSL 型の定義元は `src/domain/dsl-types/（公開エントリ: index.ts）` に固定し、`renderer/types` は再エクスポート責務に限定する。
...
1. 共有 DSL 型の追加・変更は **最初に `src/domain/dsl-types/（公開エントリ: index.ts）`** を更新する。
...
|--------|------------------------------|--------------|
| **維持（当面）** | `src/renderer/types.ts` を **thin facade** として残す。共有 DSL の意味論の正本は引き続き **`src/domain/dsl-types/（公開エントリ: index.ts）`**。 | インベントリの集計が大きく変わる・または下記「削除準備」の条件が揃いそうなとき。 |
