...
**進捗（2026-03-21）:** `test:all:ci` と CI ジョブの対応、および GitHub branch protection の**チェックリスト**を `docs/ci-quality-gate.md` に文書化済み（**実リポジトリでの必須化は管理者が上記チェックリストに従って実施**）。分類タグ・PR テンプレは別チケット（T-044 等）で追補する。
