...
State which mechanism detects which kind of `renderer/types` backflow or representative import-boundary drift so the boundary is not enforced by review alone.
...
| `npm run check:import-graph` | Representative cross-lane boundary drift (`renderer -> exporters`, `domain -> vscode`, non-allowlisted `exporters -> renderer`) | Whole repo via `scripts/check-import-graph-boundaries.cjs` | pass | Static analysis guard for 4-lane boundary shape with explicit allowlist |
...
- Representative cross-lane import drift is mechanically fixed by a dedicated script, not review memory alone.
...
| New cross-lane import drift (`renderer -> exporters`, unexpected `exporters -> renderer`, `domain -> vscode`) | `check:import-graph` | Remove the edge or route it through a documented shared bridge / dedicated ticket |
