...
If identity cannot be established with the deterministic rules already fixed in `docs/diff-ir-identity-policy.md`, the engine must fall back to remove-plus-add rather than emit a speculative move.
...
- `docs/diff-ir-identity-policy.md` remains the authority for whether two nodes are the same logical node
