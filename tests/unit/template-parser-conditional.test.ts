...
describe('TemplateParser - $if構文テスト', () => {
  ...
  before(() => {
    ...
    fs.writeFileSync(
      path.join(baseDir, 'conditional-basic.template.yml'),
      `- $if:
...
- $if:
    ...
    fs.writeFileSync(
      path.join(baseDir, 'conditional-nested.template.yml'),
      `- $if:
    ...
    // 複雑な条件式のテスト用テンプレート
    fs.writeFileSync(
      path.join(baseDir, 'conditional-complex.template.yml'),
      `- $if:
    condition: "true"
    template:
      - Text:
...
- $if:
    ...
    fs.writeFileSync(
      path.join(baseDir, 'conditional-with-include.template.yml'),
      `- $if:
    condition: "$params.showHeader"
    template:
      - $include:
  ...
  describe('基本的な$if構文', () => {
    ...
    it('一部の条件のみ真の場合、対応するテンプレートのみ展開する', async () => {
      const main = `- $include:
    template: "conditional-basic.template.yml"
    params:
      showHeader: true
  ...
  describe('ネストした$if構文', () => {
    ...
    params:
      ...
      // Containerが存在することを確認
      const containerComponent = components.find((comp: any) => comp.Container);
      expect(containerComponent).to.exist;
      expect(containerComponent.Container.id).to.equal('section');
      
      // タイトルとコンテンツが存在することを確認
      const titleComponent = containerComponent.Container.children.find((comp: any) => comp.Text && comp.Text.variant === 'h2');
      expect(titleComponent).to.exist;
      expect(titleComponent.Text.value).to.equal('セクションタイトル');
      
      const contentComponent = containerComponent.Container.children.find((comp: any) => comp.Text && comp.Text.variant === 'body');
      expect(contentComponent).to.exist;
      expect(contentComponent.Text.value).to.equal('セクションコンテンツ');
    });

    it('外側の条件が偽の場合、内側の条件も処理されない', async () => {
      const main = `- $include:
    template: "conditional-nested.template.yml"
    params:
      showSection: false
  ...
  describe('複雑な条件式', () => {
    ...
    params:
      ...
      const alwaysText = components.find((comp: any) => comp.Text && comp.Text.value === '常に表示される');
      expect(alwaysText).to.exist;
      
      // "表示されない"テキストが存在しないことを確認
      const neverText = components.find((comp: any) => comp.Text && comp.Text.value === '表示されない');
      expect(neverText).to.be.undefined;
      
      // ユーザータイプテキストが存在することを確認
      const userTypeText = components.find((comp: any) => comp.Text && comp.Text.value === 'ユーザータイプ: admin');
      expect(userTypeText).to.exist;
      
      // 管理者ボタンが存在することを確認
      const adminButton = components.find((comp: any) => comp.Button && comp.Button.label === '管理者機能');
      expect(adminButton).to.exist;
    });

    it('空文字列やnullの条件を正しく評価する', async () => {
      const main = `- $include:
    template: "conditional-complex.template.yml"
    params:
      userType: ""
  ...
  describe('$includeと$ifの組み合わせ', () => {
    it('$if内で$includeを使用できる', async () => {
      const main = `- $include:
