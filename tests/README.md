...
## HtmlExporter lane taxonomy（T-20260420-001）

- **Primary（唯一）**: `HtmlExporter` の HTML 出力。`useReactRender` を省略するか **`true`** を渡す。export / provider / preview 整合の正本。
- **`useReactRender: false`**: **未対応** — `HtmlExporter` は **`[HtmlExporter:FALLBACK_REMOVED]`** で拒否する（互換レーン削除）。
