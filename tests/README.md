...
- **T-021（削除条件正本）**: 互換レーンを**いつ消してよいか**は [t021-fallback-removal-criteria.md](../docs/current/theme-export-rendering/t021-fallback-removal-criteria.md)（構造・品質・運用ゲートと契約棚卸し）。
- **T-022（観測）**: 互換レーン通過時は **`TEXTUI_LOG_LEVEL=debug`** で `textui.html_exporter.fallback_lane` の structured debug が出る。契約は `tests/unit/html-exporter-fallback-structured-log.test.js`。
