...
2. **サンプル**: `sample/01-basic/` に基本サンプル、`sample/02-theme/`・`sample/03-include/`・`sample/05-theme-inheritance/` にテーマ・include・テーマ継承の例があります。
3. **プレビュー表示**: コマンドパレット（`Ctrl+Shift+P`）で`TextUI: Open Preview`を実行
4. **自動レイアウト**: プレビューが右側、編集画面が左側に自動配置され、編集に集中できます
5. **リアルタイム更新**: ファイル保存時に自動でプレビューが更新されます
...
v0.3.0 から、テーマファイルで `extends` によりベーステーマを継承できます。サンプルは `sample/05-theme-inheritance/` を参照してください。
...
TextUI Designerでは、コマンドパレットから簡単に用途別テンプレート（フォーム・ダッシュボード・ステップフォーム）を新規作成できます。
...
3. **テンプレート種別を選ぶ**  
   「フォーム（会員登録）」「ダッシュボード」「ステップフォーム」から用途に合うテンプレート種別を選択します。
...
- 各テンプレートには説明コメントと最小テーマ定義（`theme.tokens.color`）が含まれており、作成直後から即プレビューできます。
...
## 品質・テスト状況
- **✅ 現行バージョン**: v0.3.0（2026-03-01）
- **✅ 249個の自動テストが全てパス**: unit / integration / e2e / regression を継続実行して品質を担保
